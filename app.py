...
def load_and_parse_multiple_tables(uploaded_file):
    ...
    try:
        ...
        for i, (key, start_idx) in enumerate(sorted_starts):
            ...
            # 컬럼명 정제 (중요: 공백 제거)
            sub_df.columns = sub_df.columns.astype(str).str.strip()
            
            # 첫 컬럼명 통일 ('Mall')
            cols = list(sub_df.columns)
            if pd.isna(cols[0]) or cols[0] == 'nan' or cols[0] == '':
                cols[0] = 'Mall'
            sub_df.columns = cols
            
            # 'Mall' 컬럼 기준 NaN 제거 (데이터 없는 행 삭제)
...
def process_timeseries_data(df):
    ...
    # 'Mall' 컬럼이 있는지 확인 (index reset 후)
    if 'Mall' not in df_reset.columns:
         df_reset.rename(columns={'index': 'Mall'}, inplace=True)
    ...
    if data_dict:
        ...
        with tab3:
            ...
            if df_male is not None and df_female is not None:
                valid_malls_demo = [m for m in selected_malls if m in df_male.index and m in df_female.index]
                ...
                if valid_malls_demo:
                    ...
                    # [수정] 남성/여성 각각 독립적으로 컬럼 추출 (오류 방지)
                    age_cols_m = [c for c in df_male_filter.columns if c != '전체']
                    age_cols_f = [c for c in df_female_filter.columns if c != '전체']
                    
                    with col3_1:
                        st.markdown("**🚹 남성 연령 분포**")
                        if age_cols_m:
                            df_m_melt = df_male_filter[age_cols_m].reset_index().melt(id_vars='Mall', var_name='Age', value_name='Ratio')
                            fig_m_age = px.bar(df_m_melt, x='Ratio', y='Mall', color='Age', orientation='h',
                                            color_discrete_sequence=px.colors.sequential.Blues,
                                            title="Male Age Distribution")
                            fig_m_age.update_layout(height=400, xaxis_title="비중(%)", yaxis_title=None)
                            st.plotly_chart(fig_m_age, use_container_width=True)
                        else:
                            st.warning("남성 연령 데이터를 찾을 수 없습니다.")

                    with col3_2:
                        st.markdown("**🚺 여성 연령 분포**")
                        if age_cols_f:
                            # 여기서 여성 데이터 컬럼(age_cols_f)을 직접 사용
                            df_f_melt = df_female_filter[age_cols_f].reset_index().melt(id_vars='Mall', var_name='Age', value_name='Ratio')
                            fig_f_age = px.bar(df_f_melt, x='Ratio', y='Mall', color='Age', orientation='h',
                                            color_discrete_sequence=px.colors.sequential.Reds,
                                            title="Female Age Distribution")
                            fig_f_age.update_layout(height=400, xaxis_title="비중(%)", yaxis_title=None)
                            st.plotly_chart(fig_f_age, use_container_width=True)
                        else:
                            st.warning("여성 연령 데이터를 찾을 수 없습니다.")
                else:
                    st.info("선택한 브랜드의 인구 통계 데이터가 부족합니다.")
            else:
                st.warning("인구 통계 데이터 섹션을 찾을 수 없습니다.")

    else:
        st.error("데이터 파일 형식을 인식하지 못했습니다.")
else:
